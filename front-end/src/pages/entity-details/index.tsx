...
export function EntityDetails() {
  ...
  useEffect(() => {
    ...
    const fetchDocs = async () => {
      if (data && state && state.permission === DataHashStatus.Approved && data.dataHash !== "") {
        try {
          setDocLoading(true);
          const res = await getDataFromIpfs(data.dataHash);
          if (isMounted) {
            // Ensure res is an array before setting to documents state
            if (Array.isArray(res)) {
              ...
              try {
                ...
                setDocuments(Array.isArray(parsedRes) ? parsedRes : []);
              } catch {
                setDocuments([]);
              }
            } else {
              // Set to empty array if res is not in expected format
              console.error('Invalid data format retrieved from IPFS', res);
              setDocuments([]);
            }
          }
        } catch (error) {
