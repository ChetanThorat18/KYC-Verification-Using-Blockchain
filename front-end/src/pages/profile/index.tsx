...
export function ProfilePage() {
  ...
  const uplaodToIpfs = async () => {
    setSaveLoader(true);
    try {
      const jsonData = JSON.stringify(Array.isArray(allDocs) ? allDocs : []);
        ...
        try {
          ...
          if (Array.isArray(result)) {
            ...
            try {
              ...
              setAllDocs(Array.isArray(parsedResult) ? parsedResult : []);
            } catch {
              setAllDocs([]);
            }
          } else {
            console.error('Invalid data format retrieved from IPFS', result);
            setAllDocs([]);
          }
        } catch (error) {
          console.log(error);
